...
void Level::Init()
{
	m_TriangleTexture = Vortex::Texture2D::Create("/home/pc/dev/engine/Sandbox/assets/Textures/Triangle.png");
	m_Player.LoadAssets();
...
void Level::OnUpdate(Vortex::TimeStep ts)
{
...
void Level::OnRender()
{
	...
	// Background
	Vortex::Renderer2DGame::DrawQuad({ playerPos.x, 0.0f, -0.8f }, { 50.0f, 50.0f }, { 0.3f, 0.3f, 0.3f, 1.0f });

	// Floor and ceiling
	Vortex::Renderer2DGame::DrawQuad({ playerPos.x,  34.0f }, { 50.0f, 50.0f }, color);
	Vortex::Renderer2DGame::DrawQuad({ playerPos.x, -34.0f }, { 50.0f, 50.0f }, color);

	for (auto& pillar : m_Pillars)
	{
		Vortex::Renderer2DGame::DrawQuad(pillar.TopPosition, pillar.TopScale, glm::radians(180.0f), m_TriangleTexture, color);
		Vortex::Renderer2DGame::DrawQuad(pillar.BottomPosition, pillar.BottomScale, 0.0f, m_TriangleTexture, color);
	}

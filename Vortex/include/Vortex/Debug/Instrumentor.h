	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				// profiling output.
				if (Logging::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				{
...
#define VX_PROFILE 0
	...
	#define VX_PROFILE_BEGIN_SESSION(name, filepath) ::Hazel::Instrumentor::Get().BeginSession(name, filepath)
	#define VX_PROFILE_END_SESSION() ::Hazel::Instrumentor::Get().EndSession()
	#define VX_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = ::Hazel::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
